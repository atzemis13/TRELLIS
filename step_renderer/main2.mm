...
bool IsIso(const Handle(Geom2d_Curve)& curve, double& val, bool& isU, double first, double last) {
    if (curve.IsNull()) return false;
    
    gp_Pnt2d p1 = curve->Value(first);
    gp_Pnt2d p2 = curve->Value((first + last) / 2.0);
    gp_Pnt2d p3 = curve->Value(last);
    
    double tol = 1e-5; 
    
    // Check U-iso (U is constant)
    if (std::abs(p1.X() - p2.X()) < tol && std::abs(p1.X() - p3.X()) < tol) {
        val = p1.X();
        isU = true;
        return true;
    }
    
    // Check V-iso (V is constant)
    if (std::abs(p1.Y() - p2.Y()) < tol && std::abs(p1.Y() - p3.Y()) < tol) {
        val = p1.Y();
        isU = false;
        return true;
    }
...
TopoDS_Shape ExtractKnots(const TopoDS_Shape& shape) {
    ...
    for (; expl.More(); expl.Next()) {
        ...
        Handle(Geom_Surface) basisSurf = GetBasisSurface(surf);

        if (basisSurf->IsKind(STANDARD_TYPE(Geom_BSplineSurface))) {
            Handle(Geom_BSplineSurface) bspline = Handle(Geom_BSplineSurface)::DownCast(basisSurf);
            
            // Collect boundary iso values
            std::vector<double> boundaryU, boundaryV;
            TopExp_Explorer edgeExpl(face, TopAbs_EDGE);
            for (; edgeExpl.More(); edgeExpl.Next()) {
                const TopoDS_Edge& edge = TopoDS::Edge(edgeExpl.Current());
                double first, last;
                Handle(Geom2d_Curve) c2d = BRep_Tool::CurveOnSurface(edge, face, first, last);
                double val;
                bool isU;
                if (IsIso(c2d, val, isU, first, last)) {
                    if (isU) boundaryU.push_back(val);
                    else boundaryV.push_back(val);
                }
            }

            const TColStd_Array1OfReal& uKnots = bspline->UKnots();
            for (int i = uKnots.Lower(); i <= uKnots.Upper(); ++i) {
                double k = uKnots.Value(i);
                bool isBoundary = false;
                for (double b : boundaryU) {
                    if (std::abs(k - b) < 1e-5) { isBoundary = true; break; }
                }
                if (isBoundary) continue;

                Handle(Geom_Curve) curve = bspline->UIso(k);
            ...
            for (int i = vKnots.Lower(); i <= vKnots.Upper(); ++i) {
                double k = vKnots.Value(i);
                bool isBoundary = false;
                for (double b : boundaryV) {
                    if (std::abs(k - b) < 1e-5) { isBoundary = true; break; }
                }
                if (isBoundary) continue;

                Handle(Geom_Curve) curve = bspline->VIso(k);
                TopoDS_Edge edge = BRepBuilderAPI_MakeEdge(curve);
                edge.Move(loc);
                builder.Add(compound, edge);
            }
...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        else if (arg == "--knots") args.render_knots = true;
    }
